...
export function Navbar() {
  ...
  const [step, setStep] = useState<"phone" | "otp" | "login" | "register">("login");
  ...
  useEffect(() => {
    if (window.google) {
      window.google.accounts.id.initialize({
        client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID || '97327593629-s68070ilu69bq35fg73n298nt834v924.apps.googleusercontent.com',
  ...
  const handleEmailRegister = async () => {
    ...
    setLoading(true);
    try {
      const response = await apiClient.registerEmail(email, password, name);
      if (response.success) {
        const userData = response.user;
        setUser(userData);
        setShowLoginModal(false);
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative">
              ...
              <>
                <div className="mb-4">
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:bg-gray-100"
                  />
                </div>
            ...
            {/* Email Register Form */}
            {step === "register" && authMethod === "email" && (
              <>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Name
            ...
            {/* Phone Number Entry */}
            {((step === "login" || step === "register") && authMethod === "phone") && (
              <>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Phone Number
            ...
            {/* Divider - Only show on login/register steps */}
            {(step === "login" || step === "register") && (
            ...
            {/* Google Login - Only show on login/register steps */}
            {(step === "login" || step === "register") && (
              <>
